###
# This is the code using the CBC mode of AES encrypting
###
...
def encrypt(plaintext):
    ...
    number_blocks = int(len(blocks) / BLOCK_SIZE)
    ...
    for block_number in range(number_blocks):
        ...
        block_as_byte = blocks.encode('ascii')
        if block_number == 0:
            xor_block = xor(block_as_byte[start_of_actual_block:end_of_actual_block], IV)
        else:
            xor_block = xor(block_as_byte[start_of_actual_block:end_of_actual_block], previous_cipher_block)
...
def decrypt(ciphertext):
    ...
    for block_number in range(number_blocks):
        ...
        # Computing the previous block : where it starts and where it ends
        end_previous_block_nb = block_number * BLOCK_SIZE
        start_previous_block_nb = end_previous_block_nb - BLOCK_SIZE

        # If 1st block, I use the IV
        if block_number == 0:
            decrypted_msg = cipher.decrypt(ciphertext[start_of_actual_block:end_of_actual_block])
            xor_block = xor(decrypted_msg, IV)
        # else I use the previous block
        else:
            decrypted_msg = cipher.decrypt(ciphertext[start_of_actual_block:end_of_actual_block])
            xor_block = xor(decrypted_msg, ciphertext[start_previous_block_nb:end_previous_block_nb])
...
def main():
    ...
    with open('./Personal-Ciphertext-CBC.txt', 'w') as cipherfile:
        # Iterating on each line to encrypt it when it is not a \n
